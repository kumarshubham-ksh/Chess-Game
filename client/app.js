document.addEventListener("DOMContentLoaded", function () {
  ...
  function initializeBoard(state) {
    ...
    document.querySelector(
      ".turn-indicator"
    ).textContent = `Player ${state.turn}'s Turn`;
  }
  ...
  function switchTurn() {
    ...
    document.querySelector(
      ".turn-indicator"
    ).textContent = `Player ${turn}'s Turn`;
  }
  ...
  board.addEventListener("click", function (event) {
    ...
    if (selectedBox) {
      ...
      if (
        selectedPiece &&
        isValidMove(
          boxes.indexOf(selectedBox),
          boxes.indexOf(clickedBox),
          selectedPiece
        ) &&
        isMoveAllowed(boxes.indexOf(selectedBox), boxes.indexOf(clickedBox))
      ) {
      ...
      if (
        clickedBox.getAttribute("data-piece") &&
        clickedBox.getAttribute("data-piece")[0] === turn
      ) {
